...
#ifdef NUBIA_TOUCH_SYNAPTICS
static int synaptics_pinctrl_select_state(struct synaptics_rmi4_data *rmi4_data,
		bool active);
...
#ifdef NUBIA_TOUCH_SYNAPTICS
static struct device_attribute nubia_attrs[] = {
...
{
#ifdef NUBIA_TOUCH_SYNAPTICS
	struct synaptics_rmi4_data *rmi4_data = g_rmi4_data;
	if (NULL == rmi4_data) {
...
#ifdef CONFIG_NUBIA_LCD_KEEP
	nubia_wakeup_gesture = rmi4_data->enable_wakeup_gesture;
	pr_info("%s: nubia_wakeup_gesture=%d\n", __func__, nubia_wakeup_gesture);
#endif
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			/*modify by wuyujing start*/
			input_report_abs(rmi4_data->input_dev,
					ABS_MT_POSITION_X, x);
			input_report_abs(rmi4_data->input_dev,
					ABS_MT_POSITION_Y, y);
			/*modify by wuyujing end*/
#ifdef REPORT_2D_W
...
static int synaptics_pinctrl_select_state(struct synaptics_rmi4_data *rmi4_data,
			bool active)
{
	struct synaptics_dsx_pinctrl pin_info = rmi4_data->hw_if->board_data->pinctrl_info;
	int ret = -EINVAL;

	if (pin_info.pinctrl == NULL || pin_info.pin_active == NULL
		|| pin_info.pin_suspend == NULL) {
		pr_err("%s: pinctrl is invalid, skip.\n",__func__);
		return ret;
	}

	ret = pinctrl_select_state(pin_info.pinctrl, active ? pin_info.pin_active : pin_info.pin_suspend);
	if (ret < 0)
		pr_err("%s: pinctrl select %d state error:%d\n", __func__, active, ret);
...
{
	int retval;
	unsigned char attr_count, nubia_attr_count;
	struct synaptics_rmi4_data *rmi4_data;
...
#ifdef NUBIA_TOUCH_SYNAPTICS
	for (nubia_attr_count = 0; nubia_attr_count < ARRAY_SIZE(nubia_attrs); nubia_attr_count++) {
		retval = sysfs_create_file(&rmi4_data->input_dev->dev.kobj,
			&nubia_attrs[nubia_attr_count].attr);
		if (retval < 0) {
			dev_err(&pdev->dev,
				"%s: Failed to create sysfs attributes\n",
				__func__);
			goto nubia_err_sysfs;
		}
	}
#endif
#ifdef USE_DATA_SERVER
...
nubia_err_sysfs:
	for (nubia_attr_count--; nubia_attr_count >= 0; nubia_attr_count--) {
		sysfs_remove_file(&rmi4_data->pdev->dev.parent->kobj,
				&nubia_attrs[nubia_attr_count].attr);
	}
		...
		unsigned long event, void *data)
{
	...
	/* repair breaking screencast tp unusefull, start */
	struct msm_drm_notifier *eventdata = data;
	pr_info("%s: morgen eventid=%d\n", __func__, eventdata->id);
	if (!eventdata || (eventdata->id != 0)) {
		return 0;
	}
	/* repair breaking screencast tp unusefull, end */
...
#ifdef NUBIA_TOUCH_SYNAPTICS
		synaptics_pinctrl_select_state(rmi4_data, true);
#endif
...
#endif
#ifdef NUBIA_TOUCH_SYNAPTICS
		synaptics_pinctrl_select_state(rmi4_data, false);
#endif
		synaptics_rmi4_irq_enable(rmi4_data, false, false);
		synaptics_rmi4_sleep_enable(rmi4_data, true);
		synaptics_rmi4_free_fingers(rmi4_data);
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (rmi4_data->stay_awake)
		return 0;
#ifdef NUBIA_TOUCH_SYNAPTICS
	/*boot up unblank will call resume, TP don't suspend*/
	if (!rmi4_data->suspend)
		return 0;
#endif
	if (rmi4_data->enable_wakeup_gesture) {
