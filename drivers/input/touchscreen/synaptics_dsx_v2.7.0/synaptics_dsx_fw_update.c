...
static ssize_t nubai_fwu_sys_ic_ver_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
#ifdef NUBIA_TOUCH_SYNAPTICS
		if (NULL == rmi4_data->hw_if->board_data->fw_image_name) {
			pr_err("%s: fw_image_name is NULL\n", __func__);
			goto exit;
		}

		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				rmi4_data->hw_if->board_data->fw_image_name, strlen(rmi4_data->hw_if->board_data->fw_image_name) + 1,
				strlen(rmi4_data->hw_if->board_data->fw_image_name) + 1);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
...
#ifdef NUBIA_TOUCH_SYNAPTICS
static int nubai_fwu_sys_ic_get_firmware_id(unsigned int *firmware_id)
{
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
	struct synaptics_rmi4_device_info rmi = {0};
	int retval = 0;
...
#define F01_BUID_ID_OFFSET 18
	...
	*firmware_id = (unsigned int)rmi.build_id[0] +
		...
		struct device_attribute *attr, char *buf)
{
	...
	nubai_fwu_sys_ic_get_firmware_id(&firmware_id);
	fwu_get_device_config_id();

	return snprintf(buf, PAGE_SIZE, "firmware_id:%u\nconfig_id:%02x%02x%02x%02x\n",
		fwu->rmi4_data->firmware_id, fwu->config_id[0], fwu->config_id[1],
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (nubia_attr_count = 0; nubia_attr_count < ARRAY_SIZE(nubia_attrs); nubia_attr_count++) {
		retval = sysfs_create_file(&rmi4_data->pdev->dev.parent->kobj,
