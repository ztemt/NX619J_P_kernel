...
static void tmd2725_read_prox(struct tmd2725_chip *chip)
{
	chip->shadow[TMD2725_REG_PDATA] = 0;
...
static void tmd2725_get_prox(struct tmd2725_chip *chip)
{
	...
	if (chip->prx_inf.detected == PROX_FAR) {
		...
		if (chip->prx_inf.raw < chip->params.prox_th_min) {
			...
			SENSOR_LOG_INFO("chip->prx_inf.raw :%d prox_thres_far:%d",chip->prx_inf.raw ,chip->params.prox_thres_far);
			if (chip->prox_state == PROX_STATE_NONE) {
				chip->prx_inf.detected = PROX_FAR;
				tmd2725_report_prox(chip);
				if (chip->params.prox_th_max != PS_DATA_MAX) {
					...
					chip->params.prox_th_min = 0;
					chip->params.prox_th_max = chip->params.prox_thres_near;
					SENSOR_LOG_INFO("prx_th_max :%d prx_th_min :%d",chip->params.prox_th_max,chip->params.prox_th_min);
				} else {
					update_thresholds = true;
...
static int resume_error_restore(struct tmd2725_chip *chip)
{
	u8 res = 0, status = 0;
	u8 *sh = chip->shadow;
	if(chip->resume_error)
	{
		res = ams_i2c_read(chip->client, TMD2725_REG_STATUS, &chip->shadow[TMD2725_REG_STATUS]);
		if(res <= 0)
		{
			SENSOR_LOG_ERROR("i2c read failed while restore resume error\n");
			return -1;
		}
		status = chip->shadow[TMD2725_REG_STATUS];
		if(status != 0)
			ams_i2c_write(chip->client, sh, TMD2725_REG_STATUS, status);
		chip->resume_error = false;
		SENSOR_LOG_ERROR("resume restore finished\n");
	}
	return 0;
}
...
static int tmd2725_prox_thres_calibrate(struct tmd2725_chip *chip)
{
    ...
    u8 prox_thres_near;
	SENSOR_LOG_INFO("enter\n");
	prox_thres_near = tmd2725_mean_prox_calc(chip);
...
static int tmd2725_prox_thres_to_persist(struct tmd2725_chip *chip)
{
	int ret;
	u8 prox_buf[2];
	SENSOR_LOG_INFO("enter\n");
...
static ssize_t tmd2725_prox_thres_to_persist_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	int val;
	...
	if (rc < 0)
		return rc;
	if (val) {
		rc = tmd2725_prox_thres_to_persist(chip);
	}
	return rc;
}
...
static struct device_attribute tmd2725_prox_attrs[] = {
	...
	__ATTR(prox_thres_to_persist, 0220, NULL, tmd2725_prox_thres_to_persist_store),
};
...
static int resume_error_handle(struct tmd2725_chip *chip)
{
	SENSOR_LOG_ERROR("proximity interrupt handled before kernel ready\n");
	if(chip->prx_inf.detected == PROX_NEAR)
	{
		sensor_irq_enable(chip, false, false);
		chip->resume_error = true;
		chip->prx_inf.detected = PROX_FAR;
		tmd2725_report_prox(chip);
		SENSOR_LOG_ERROR("close irq and report far away directly\n");
	}
	return 0;
}
