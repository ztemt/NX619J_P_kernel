...
/* debug log level */
static fp_debug_level_t debug_level = ALL_LOG;//DBG_LOG;
                             ...
                             unsigned long event, void *data)
{
    ...
    blank = *(int *)evdata->data;
     ...
     LOG_MSG_DEBUG(INFO_LOG, "[%s] enter, event=%d blank=%d\n", __func__, (int)event, blank);
	 if (event != FB_EARLY_EVENT_BLANK_FP)
	 {
	 	LOG_MSG_DEBUG(INFO_LOG, "[%s] event != FB_EARLY_EVENT_BLANK_FP\n", __func__);
	 	return 0;
	 }

	 if (!evdata || !evdata->data || !fp_dev)
	 {
		LOG_MSG_DEBUG(INFO_LOG, "[%s] NULL pointer!!\n", __func__);
		return 0;
	 }

    switch (blank) {
    case FB_BLANK_UNBLANK_FP:
        LOG_MSG_DEBUG(INFO_LOG, "[%s] LCD ON\n", __func__);
        fp_dev->scr_off = 0;
        silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_ON);
        break;

    case FB_BLANK_POWERDOWN_FP:
...
static int silfp_init(struct silfp_data *fp_dev)
{
    ...
    msm_drm_panel_register_client(&fp_dev->notif);
#endif /* CONFIG_HAS_EARLYSUSPEND */
...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	//LOG_MSG_DEBUG(INFO_LOG, "[%s] cmd=%d\n", __func__, cmd);
...
static int silfp_probe(struct platform_device *spi)
{
    struct silfp_data	*fp_dev;
    int			status = 0;
#ifdef FEATURE_NUBIA_FINGERPRINT_AUTODETECT
    int id_gpio_val = 0;
#endif
    //unsigned long		minor;
...
#ifdef FEATURE_NUBIA_FINGERPRINT_AUTODETECT
    #define NUBIA_SENSORS_FP_ID_GPIO 86
    fp_dev->avdd_ldo = regulator_get(&fp_dev->spi->dev, "avdd");
    if (IS_ERR(fp_dev->avdd_ldo)) {
	status = PTR_ERR(fp_dev->avdd_ldo);
	LOG_MSG_DEBUG(ERR_LOG, "Regulator get failed vdd status =%d\n", status);
	return status;
    }
    status = gpio_request(NUBIA_SENSORS_FP_ID_GPIO, "NUBIA_ID_PIN");
    if (status < 0) {
	LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, status =%d",__func__, NUBIA_SENSORS_FP_ID_GPIO, status);
	return status;
    }
    silfp_hw_poweron(fp_dev);
    mdelay(5);
    id_gpio_val = gpio_get_value(NUBIA_SENSORS_FP_ID_GPIO);
    if (id_gpio_val) {
    	LOG_MSG_DEBUG(ERR_LOG, "id_gpio_val = %d. It's fingerprint chip of vendor : SILEAD\n", id_gpio_val);
        gpio_free(NUBIA_SENSORS_FP_ID_GPIO);
	silfp_power_deinit(fp_dev);
    }
    else {
    	LOG_MSG_DEBUG(ERR_LOG, "id_gpio_val = %d. It's fingerprint chip of vendor : GOODIX, now driver will exit\n", id_gpio_val);
        gpio_free(NUBIA_SENSORS_FP_ID_GPIO);
	silfp_power_deinit(fp_dev);
	kfree(fp_dev);
	fp_dev = NULL;
