...
static void tfa_haptic_pll_off(struct work_struct *work)
{
	...
	//	pr_info("tfa_mclk off--");
	//}
}

static void tfa_haptic_pll_off_sync(struct tfa9xxx *drv)
{
...
void tfa9xxx_haptic_clock(struct tfa9xxx *drv, bool on)
{
	...
	if (on) {
		...
		if (drv->clk_users == 0)
			...
			tfa_haptic_pll_off_sync(drv);
	}
	mutex_unlock(&drv->power_mutex);
...
static void tf9xxx_update_led_class(struct work_struct *work)
{
	...
	dev_dbg(&drv->i2c->dev, "%s: type: %d, index: %d, state: %d, duration = %d\n",
			__func__,	(int)obj->type, obj->index,
			obj->state, data->duration);
#else
	dev_dbg(&drv->i2c->dev, "%s: type: %d, index: %d, state: %d\n", __func__, (int)obj->type, obj->index, obj->state);
#endif
...
static void tfa9xxx_stop_sync(struct tfa9xxx *drv, struct drv_object *obj)
{
	struct haptic_data *data;
	BUG_ON(!drv->patch_loaded);
	data = &drv->tfa->hap_data;
                                          ...
                                          const char *buf, size_t count)
{
	...
	dev_info(&drv->i2c->dev, "%s: update_object_index=%d, val=%d\n",
	...
	if (state == 0) {
		if (obj->state == STATE_STOP) {
			dev_dbg(&drv->i2c->dev, "tfa9xxx already stop, return!\n");
			return count;
		}
		obj->state = STATE_STOP;
	} else {
	...
	rc = hrtimer_cancel(&obj->active_timer);
	dev_dbg(&drv->i2c->dev, "tfa hrtimer_cancel return:%d\n", rc);
	...
	if (obj->state != STATE_STOP) {
		...
		dev_info(&drv->i2c->dev, "%s: start active %d timer of %d msecs\n",
		        __func__, (int)obj->type, value);
		schedule_work(&obj->update_work);
	} else {
		tfa9xxx_stop_sync(drv, obj);
	}
...
int tfa9xxx_bck_stops(struct tfa9xxx *drv)
{
	...
	dev_info(&drv->i2c->dev, "%s: bck_running=%d, mclk=%p\n", __func__,
	        (int)drv->bck_running, drv->mclk);
...
static ssize_t tfa9xxx_store_amplitude(struct device *dev,
                                         struct device_attribute *attr,
                                         const char *buf, size_t count)
{
	int val;
	...
	tfa->hap_data.amplitude = val;
	return count;
}

static ssize_t tfa9xxx_show_amplitude(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return tfa9xxx_show_value(dev, attr, buf);
}
...
static struct device_attribute tfa9xxx_haptic_attrs[] = {
    ...
    __ATTR(amplitude, 0664, tfa9xxx_show_amplitude, tfa9xxx_store_amplitude),
};
