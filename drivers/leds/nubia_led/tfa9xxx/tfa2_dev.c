...
{
	if ((status < Tfa98xx_no_dsp_response) || (status > Tfa98xx_I2C_Req_Calib_Failed))
		status = Tfa98xx_I2C_Req_Calib_Failed + 1; /* Unspecified error */

	/*  enum Tfa98xx_Status_ID starts at -1 */
...
{
        "TFA_STATE_NONE",     /**< none or unknown or invalid */
...
int tfa2_dev_get_revid(struct tfa2_device *tfa) //TODO remove
{
	...
	rc = tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_REV);
	if (rc < 0)
		return rc;

	dev_info(&tfa->i2c->dev, "HW rev: 0x%04x\n", (uint16_t)rc );
...
int tfa2_i2c_get_revid(struct i2c_client *i2c)
{
	...
	dev_info(&i2c->dev, "HW rev: 0x%04x\n", (uint16_t)rc );
...
int tfa2_dev_start_cf(struct tfa2_device *tfa)
{
	...
	do {
		...
		if ( rc < 0 )
			return rc;
	} while (loop-- && !(tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_ACS)));
	dev_info(&tfa->i2c->dev, "%s,  loop = %d \n",__FUNCTION__, loop);
...
int tfa2_dev_start(struct tfa2_device *tfa, int next_profile, int vstep)
{
	...
	dev_info(&tfa->i2c->dev, "%s ", __FUNCTION__);
	dev_info(&tfa->i2c->dev, "HW:%s ", tfa->need_hw_init?"cold":"warm");
	dev_info(&tfa->i2c->dev, "SB:%s ", tfa->need_sb_config?"cold":"warm");
	if (tfa->need_hb_config) {
		dev_info(&tfa->i2c->dev, "HB:%s %s\n",
				(tfa->need_hb_config & TFA_HB_ROLE_MASK) == tfa_hb_lra ? "lra" : "ls",
				(tfa->need_hb_config & tfa_hb_ready) ?"warm":"cold"	); /* ready is warm */
	}
	else {
		dev_info(&tfa->i2c->dev, "HB:none\n");
	}
...
{
	dev_info(&tfa->i2c->dev, "tfa SET SBSL !!!\n");
...
int tfa2_dev_set_state(struct tfa2_device *tfa, enum tfa_state state) {
	...
	dev_info(&tfa->i2c->dev, "tfa set logical state: %s\n",
	...
	dev_info(&tfa->i2c->dev, "tfa ++ manstate %d\n", manstate);
	...
	case TFA_STATE_POWERDOWN: /* PLL in powerdown, Algo up */
		...
		count = 1000; /* TODO 400 ms worst case time out */
		while ((manstate != 0) && (count > 0)) { //TODO unify polling functions
		...
		if (count == 0) {
			dev_err(&tfa->i2c->dev, "PLL in powerdown wait timedout.\n");
			rc = -ETIME;
			break;
		}
		tfa2_i2c_write_bf(tfa->i2c ,TFA9XXX_BF_MANAOOSC, 1); /* 1 = off */
		break;
	case TFA_STATE_POWERUP: /**< PLL to powerup, Algo can be up/warm */
		...
		/* only return error for clks wait when cold */
		dev_dbg(&tfa->i2c->dev, "Waiting for DSP system stable...\n");
		rc = tfa2_dev_clock_stable_wait(tfa);
		rc = tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_MANCOLD)  ? rc : 0;
		break;
	case TFA_STATE_OPERATING: /* Amp and Algo running */
		...
		/* powerup first if not already running */
		if (manstate == 0) {
			rc = tfa2_dev_set_state(tfa, TFA_STATE_POWERUP);
			if (rc < 0)
				break;
		}
		rc = tfa2_dev_set_configured(tfa); /* will set SBSL */
		break;
	...
	case TFA_STATE_INIT_CF: /**< coolflux HW access possible (~initcf) */
		...
		manstate = tfa2_i2c_read_bf(tfa->i2c, tfa->bf_manstate);
		rc = manstate == 6 ? 0 : -EIO; /* ok or clock error return */
		break;
...
void tfa2_dev_update_config_init(struct tfa2_device *tfa)
{
	//if ( tfa->need_hw_init < 0 )
		tfa->need_hw_init = tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_ISTVDDS); /* Returns 1 when tfa  is "cold" and 0 when device is warm */

		tfa->need_cf_init = tfa->need_hw_init ; /* TODO no cf for probus devices */

	//if ( tfa->need_sb_config < 0 )
	tfa->need_sb_config = tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_SBSL) == 0; /* SBSL is  0 when fw  is not configured  else 1  */

	if ( tfa->need_hb_config < 0 ) {
		/* if we have lra_ profile prefixes we have the lra */
		if (tfa2_cnt_grep_profile_names(tfa->cnt, tfa->dev_idx, "lra_")>= 0)
			tfa->need_hb_config = tfa_hb_lra;
		else if (tfa2_cnt_grep_profile_names(tfa->cnt, tfa->dev_idx, "ls_")>= 0)
				tfa->need_hb_config = tfa_hb_ls;
		else	tfa->need_hb_config = tfa_hb_none;
	}
...
int tfa2_dev_faim_protect(struct tfa2_device *tfa, int state)
{
	...
	return tfa2_i2c_write_bf_volatile(tfa->i2c, TFA9XXX_BF_OPENMTP, (uint16_t)(state));
}
// TODO unify stable/CLKS waits : tfa_i2c_clock_stable_wait & tfa_hap_clock_stable
...
int tfa2_dev_clock_stable_wait(struct tfa2_device *tfa)
{
	...
	do {
		ready = tfa2_i2c_read_bf(tfa->i2c,tfa->bf_clks);
		if  (ready == 1)
			break;
		else if (ready < 0)
			return -EIO;
		/* msleep_interruptible(1); */ /* wait to avoid busload */
	} while (loop--);
...
int tfa2_i2c_write_cf_mem(struct i2c_client *client, uint16_t address, int32_t *input, int size, int type)
{
	int idx = 0;
	uint8_t *output;
	int rc;
...
#define CF_RPC_RETRY_NUM    0x07 /* Value calculated base on DSP FW API information */
...
int tfa2_i2c_rpc_status(struct i2c_client *i2c, int *pRpcStatus)
{
	...
	/* Set RPC result initially to Tfa98xx_no_dsp_response. In case we have comm. problems */
...
// TODO check more status ACK
	...
	if ((retry_cnt>= CF_RPC_RETRY_NUM) && !(cf_status & CF_STATUS_I2C_CMD_ACK)) {
		/* Set RPC result initially to Tfa98xx_no_dsp_response. In case we have comm. problems */
		*pRpcStatus = Tfa98xx_no_dsp_response; //TODO no_dsp_response is misleading, should be error response
		rc = -ENXIO;
	} else {
		...
		char *res_buf, size_t res_len)
{
	...
	for (tries = TFA98XX_WAITRESULT_NTRIES; tries > 0; tries--) {
		...
		if (rc  || rpc_status != Tfa98xx_I2C_Req_Busy)
...
int tfa2_i2c_dsp_msg(struct i2c_client *i2c, int wlength, char *wbuf)
{
	return tfa2_i2c_dsp_execute( i2c, wbuf, wlength, NULL, 0 );
}
 ...
 */
static int tfa2_24_to_32(int32_t * data32, uint8_t *data24, int length_bytes24)
{
	int i, idx = 0;
 ...
 *  @return total nr of bytes in the result
 */
static int tfa2_32_to_24(uint8_t *data24, int32_t * data32, int length_bytes32)
{
	int i, idx = 0;
...
int tfa2_i2c_mtp_busy(struct i2c_client *client)
{
	...
	rc = tfa2_i2c_bf_poll(client, TFA9XXX_BF_MTPB, 0, 100); //TODO mtpbusy max poll time
	if ( rc < 0 )
...
restore_state:
	...
	/* restore state  */
	rc = tfa2_dev_set_state(tfa, (save_state == 9) ? TFA_STATE_OPERATING : TFA_STATE_POWERDOWN);
	if ( rc <  0 )
		return rc;
	return 0;
}
		                       ...
		                       const uint8_t *bytes)
{
	...
	dev_info(&client->dev, "patch length is %d bytes\n", length);
...
int tfa2_calibrate_profile_start(struct tfa2_device *tfa)
{
	...
	cal_profile = tfa2_cnt_grep_profile_names(tfa->cnt, tfa->dev_idx, ".cal");
	if (cal_profile < 0) {
...
int tfa2_sb_calibrate(struct tfa2_device *tfa)
{
	...
	/* get SB calibration profile: with .cal extension */
	sbcal_profile = tfa2_cnt_grep_profile_names(tfa->cnt, tfa->dev_idx, ".cal");
	if (sbcal_profile < 0) {
		dev_err(&tfa->i2c->dev, "No [*.cal] profile found for calibration\n");
		return -EINVAL;
	}
...
int tfa2dsp_fw_get_re25(struct tfa2_device *tfa, uint8_t *buffer) {
	...
	*pbuf++ = SB_PARAM_GET_RE25C;
	...
	rc = tfa2_i2c_dsp_execute(tfa->i2c ,  (const char *)buffer,  command_length,  (char *)buffer, result_length );
