...
#define TFA9XXX_PINCTRL_STATE_DEFAULT "tfa9xxx_clk_default"
#endif

#ifdef Audio2Haptic_refGain
	int I2S_run_flag = 0;
	int Chip_run_flag = 0;
#endif
extern int F0;
...
MODULE_PARM_DESC(trace_level, "TFA9xxx debug trace level (0=off, bits:1=verbose,2=regdmesg,3=regftrace,4=timing).");

static char *dflt_prof_name = "";
module_param(dflt_prof_name, charp, S_IRUGO);

static int no_start = 1;
...
static int tfa9xxx_set_a2h_model(struct tfa9xxx *tfa9xxx, int pa_mtpF0);
...
static int tfa9xxx_dbgfs_temp_set(void *data, u64 val)
{
	...
	tfa9xxx_set_a2h_model(tfa9xxx, mtpf0);
				     ...
				     loff_t *ppos)
{
	...
	if (ready)
		err = tfa2_i2c_dsp_execute(i2c, tfa9xxx->rpc_buffer, tfa9xxx->rpc_buffer_size, buffer, count);
	mutex_unlock(&tfa9xxx->dsp_lock);
...
#ifdef Audio2Haptic_refGain
extern int send_tfa_haptic_ref_gain(void *buf, int cmd_size);

static int tfa9xxx_dbgfs_A2HDSP_no_start_get(void *data, u64 *val)
{
	struct i2c_client *i2c = (struct i2c_client *)data;
	*val = no_start;
	...
	if (val < 0) {
		dev_err(&i2c->dev, "%s: Unable to read A2HDSP_no_start: %llu\n\n",
		        __func__, *val);
		return -EIO;
	}

	dev_dbg(&i2c->dev, "%s: A2HDSP_no_start : %llu\n\n", __func__, *val);

	return 0;
}

extern int send_tfa_set_model(void *buf, int cmd_size);
static u8 model[9];
static int tfa9xxx_set_a2h_model(struct tfa9xxx *tfa9xxx, int pa_mtpF0)
{
	int ret = 0;
	...
	if(mtpex) {
		if (pa_mtpF0) {
			...
			/*this is F0 from mtp*/
			mtpf0 = tfa2_dev_mtp_get(tfa9xxx->tfa, TFA_MTP_F0);
			mtpf0 = mtpf0 / 2 + 80;
		}
		...
		model[3] = (u8)((mtpf0 >> 16) & 0xff);
		model[4] = (u8)((mtpf0 >> 8) & 0xff);
		model[5] = (u8)(mtpf0 & 0xff);
		...
		model[6] = (u8)((Q >> 16) & 0xff);
		model[7] = (u8)((Q >> 8) & 0xff);
		model[8] = (u8)(Q & 0xff);

		dev_info(&tfa9xxx->i2c->dev, "send_tfa_set_model  mtpf0:0x%x \n", mtpf0);

		ret = send_tfa_set_model(model, sizeof(model));
	}
	return ret;
}


static int tfa9xxx_dbgfs_A2HDSP_no_start_set(void *data, u64 value)
{
	struct i2c_client *i2c = (struct i2c_client *)data;
	struct tfa9xxx *tfa9xxx = i2c_get_clientdata(i2c);
	...
	if ((val != 0) && (val != 1)) {
		dev_err(&i2c->dev, "%s: Unexpected value %d\n",
		        __func__, val);
		return -EINVAL;
	}
	...
	dev_dbg(&i2c->dev, "debug: I2S_run_flag %d Chip_run_flag %d tfa9xxx->dsp_fw_state %d tfa9xxx->clk_users %d \n", 
						I2S_run_flag, Chip_run_flag, tfa9xxx->dsp_fw_state, tfa9xxx->clk_users);

	if(I2S_run_flag == 1)  //I2S already run
	{
		if((no_start == 1)&&(Chip_run_flag == 1)) //close A2H
		{
			if (tfa9xxx->bck_running && tfa9xxx->clk_users){
				tfa9xxx->clk_users--;
			}
			...
			Chip_run_flag = 0;
			dev_info(&i2c->dev, "%s:I2S already run, Close A2H, A2HDSP_no_start < %d\n", __func__, no_start);
		}
		else if((no_start==0)&&(Chip_run_flag == 0))  //open A2H
		{
			tfa9xxx_set_a2h_model(tfa9xxx, 0);
			tfa9xxx->pstream = 1;
			send_tfa_haptic_ref_gain(&tfa9xxx->a2h_gain, sizeof(tfa9xxx->a2h_gain));
			/* Start DSP */
			...
			Chip_run_flag = 1;
			dev_info(&i2c->dev, "%s:I2S already run, Open A2H, A2HDSP_no_start < %d\n", __func__, no_start);
		}
	}
	else if(I2S_run_flag == 0)   //I2S not run
	{
		if(no_start==1)
		{
			dev_dbg(&i2c->dev, "%s:I2S not run, Close A2H, A2HDSP_no_start < %d\n", __func__, no_start);
		}
		else if(no_start==0)
		{
			dev_dbg(&i2c->dev, "%s:I2S not run, Open A2H, A2HDSP_no_start < %d\n", __func__, no_start);
		}
	}
...
#ifdef Audio2Haptic_refGain
DEFINE_SIMPLE_ATTRIBUTE(tfa9xxx_dbgfs_A2HDSP_no_start_fops, tfa9xxx_dbgfs_A2HDSP_no_start_get,
                        tfa9xxx_dbgfs_A2HDSP_no_start_set, "%llu\n");
#endif
...
static void tfa9xxx_debugfs_init(struct tfa9xxx *tfa9xxx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("dsp-state", S_IRUGO|S_IWUGO, tfa9xxx->dbg_dir,
						i2c, &tfa9xxx_dbgfs_dsp_state_fops);
	debugfs_create_file("fw-state", S_IRUGO|S_IWUGO, tfa9xxx->dbg_dir,
...
#ifdef Audio2Haptic_refGain
	debugfs_create_file("A2HDSP_no_start", S_IRUGO|S_IWUGO, tfa9xxx->dbg_dir,
						i2c, &tfa9xxx_dbgfs_A2HDSP_no_start_fops);
#endif

	tfa9xxx->rpc_buffer = devm_kzalloc(&i2c->dev, TFA2_MAX_PARAM_SIZE, GFP_KERNEL);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (profile < 0) {
		dev_err(&tfa9xxx->i2c->dev, "%s: tfa9xxx_get_vstep: invalid profile %d (mixer_profile=%d, rate=%d)\n",
	...
	list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
		...
		ucontrol->value.integer.value[tfa9xxx->tfa->dev_idx] =
				tfacont_get_max_vstep(tfa9xxx->tfa, profile)
				- vstep - 1; // TODO
	}
	mutex_unlock(&tfa9xxx_mutex);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	profile = get_profile_id_for_sr(mixer_profile, tfa9xxx->rate);
	if (profile < 0) {
		dev_err(&tfa9xxx->i2c->dev, "%s: tfa9xxx_set_vstep: invalid profile %d (mixer_profile=%d, rate=%d)\n",
		        __func__, profile, mixer_profile, tfa9xxx->rate);
		return -EINVAL;
	}
	...
	list_for_each_entry(tfa9xxx, &tfa9xxx_device_list, list) {
		...
		vsteps = tfacont_get_max_vstep(tfa9xxx->tfa, profile); // TODO
		...
		if (profile == tfa9xxx->profile) {
			...
			if (ready) {
				...
				if (err) {
					dev_err(&tfa9xxx->i2c->dev, "%s: Write vstep error: %d\n",
					        __func__, err);
				} else {
					dev_dbg(&tfa9xxx->i2c->dev, "%s: Succesfully changed vstep index!\n",
					       __func__);
					change = 1;
				}
			}
		...
		dev_dbg(&tfa9xxx->i2c->dev, "%s: %d: vstep:%d, (control value: %d) - profile %d\n",
		       ...
		       struct snd_ctl_elem_info *uinfo)
{
	...
	if (profile < 0) {
		dev_err(&tfa9xxx->i2c->dev, "%s: tfa9xxx_info_vstep: invalid profile %d (mixer_profile=%d, rate=%d)\n",
		        __func__, profile, mixer_profile, tfa9xxx->rate);
		return -EINVAL;
	}
	...
	uinfo->value.integer.max = max(0, tfacont_get_max_vstep(tfa9xxx->tfa, profile) - 1); // TODO
	dev_dbg(&tfa9xxx->i2c->dev, "vsteps count: %d [prof=%d]\n",
		__func__, tfacont_get_max_vstep(tfa9xxx->tfa, profile), profile); // TODO
	return 0;
}
#endif
...
static int tfa9xxx_info_a2h_gain_ctl(struct snd_kcontrol *kcontrol,
									struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	mutex_lock(&tfa9xxx_mutex);
	uinfo->count = 1;
	mutex_unlock(&tfa9xxx_mutex);
	uinfo->value.integer.min = 65536;
	uinfo->value.integer.max = 65536000;

	return 0;
}

static int tfa9xxx_set_a2h_gain_ctl(struct snd_kcontrol *kcontrol,
									struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tfa9xxx *tfa9xxx = snd_soc_codec_get_drvdata(codec);
	...
	tfa9xxx->a2h_gain = ucontrol->value.integer.value[0];
	dev_dbg(&tfa9xxx->i2c->dev, "tfa9xxx_set_a2h_gain_ctl value set to (%d)\n", tfa9xxx->a2h_gain);

	if (tfa9xxx->dsp_init != TFA9XXX_DSP_INIT_STOPPED)
		send_tfa_haptic_ref_gain(&tfa9xxx->a2h_gain, sizeof(tfa9xxx->a2h_gain));
	else
		dev_dbg(&tfa9xxx->i2c->dev, "tfa don't send ref gain when stop");
...
static int tfa9xxx_get_a2h_gain_ctl(struct snd_kcontrol *kcontrol,
									struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tfa9xxx *tfa9xxx = snd_soc_codec_get_drvdata(codec);
	...
	mutex_lock(&tfa9xxx_mutex);
	ucontrol->value.integer.value[0] = tfa9xxx->a2h_gain;
	mutex_unlock(&tfa9xxx_mutex);
...
static int tfa9xxx_create_controls(struct tfa9xxx *tfa9xxx)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		if (tfacont_get_max_vstep(tfa9xxx->tfa, prof)) // TODO
	...
	// tfa9xxx_controls[mix_index].private_value = profs; /* save number of profiles */
	mix_index++;

	/* create mixer items for each profile that has volume */
	for (prof = 0; prof < nprof; prof++) {
		/* create an new empty profile */
		bprofile = devm_kzalloc(tfa9xxx->codec->dev, sizeof(*bprofile), GFP_KERNEL);
		if (!bprofile)
			return -ENOMEM;
		...
		if ((is_profile_in_list(bprofile->basename, bprofile->len) == 0) &&
			...
			if (tfacont_get_max_vstep(tfa9xxx->tfa, prof)) { // TODO
				...
				tfa9xxx_controls[mix_index].private_value = bprofile->item_id; /* save profile index */
				mix_index++;
			}
#endif
		}
...
#ifdef Audio2Haptic_refGain
	tfa9xxx_controls[mix_index].name = "A2H_RefValue";
	tfa9xxx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	tfa9xxx_controls[mix_index].info = tfa9xxx_info_a2h_gain_ctl;
	tfa9xxx_controls[mix_index].get = tfa9xxx_get_a2h_gain_ctl;
	tfa9xxx_controls[mix_index].put = tfa9xxx_set_a2h_gain_ctl;
	mix_index++;
#endif
...
#ifdef NUBIA_HAPTIC_TFA9XXX
static int tfa9xxx_check_calibrate(struct tfa9xxx *tfa9xxx)
{
	int ret;
	int result = 0;
	int value;
	...
	dev_info(&tfa9xxx->i2c->dev,"%s enter++\n", __func__);

	value = tfa2_i2c_read_bf(tfa9xxx->tfa->i2c, TFA9XXX_BF_MTPEX);
	if(value){
	//	mtp_f0 = tfa2_i2c_read_bf(tfa9xxx->tfa->i2c, TFA_MTP_F0);
	//	value = mtp_f0 / 2 + 80;
		dev_info(&tfa9xxx->i2c->dev, "%s: F0  = %d \n", __func__, F0);
		if(F0 > 220 && F0 < 250){
			dev_info(&tfa9xxx->i2c->dev,
		         "%s: haptic already clibrated \n", __func__);
			return 0;
		}else{ /*if clibrated fail, we clibrated again*/
			ret = tfa2_dev_mtp_set(tfa9xxx->tfa, TFA_MTP_EX, 0);
			msleep(3);
		}
	}

	ret = tfa2_dev_mtp_set(tfa9xxx->tfa, TFA_MTP_OTC, 1);
	if (ret) {
		dev_err(&tfa9xxx->i2c->dev,
		"%s: SB Calibration write OTC fail (%d)\n", __func__, ret);
		result = -EIO;
	}
	ret = tfa2_sb_calibrate(tfa9xxx->tfa);
	if (ret) {
		...
		tfa2_dev_mtp_set(tfa9xxx->tfa, TFA_MTP_EX, 1);
		dev_info(&tfa9xxx->i2c->dev,
		         "%s: SB Calibration done\n", __func__);
	}
	ret = tfa2_hap_calibrate(tfa9xxx->tfa);
...
static void tfa9xxx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (tfa9xxx_container == NULL) {
		...
		dev_info(&tfa9xxx->i2c->dev, "%.2s%.2s\n", container->version, container->subversion);
		dev_info(&tfa9xxx->i2c->dev, "%.8s\n", container->customer);
		dev_info(&tfa9xxx->i2c->dev, "%.8s\n", container->application);
		dev_info(&tfa9xxx->i2c->dev, "%.8s\n", container->type);
		dev_info(&tfa9xxx->i2c->dev, "%d ndev\n", container->ndev);
		dev_info(&tfa9xxx->i2c->dev, "%d nprof\n", container->nprof);
...
#ifdef NUBIA_HAPTIC_TFA9XXX
	err = tfa9xxx_check_calibrate(tfa9xxx);
	if (err)
		dev_err(&tfa9xxx->i2c->dev, "tfa9xxx calibrate error \n");
#endif
...
static void tfa9xxx_monitor(struct work_struct *work)
{
	...
	mutex_lock(&tfa9xxx->dsp_lock);
	error = tfa_status(tfa9xxx->tfa); // TODO
	mutex_unlock(&tfa9xxx->dsp_lock);
	if (error == Tfa98xx_Error_DSP_not_running) {
		if (tfa9xxx->dsp_init == TFA9XXX_DSP_INIT_DONE) {
			tfa9xxx->dsp_init = TFA9XXX_DSP_INIT_RECOVER;
			queue_delayed_work(tfa9xxx->tfa9xxx_wq, &tfa9xxx->init_work, 0);
		}
	}
...
static int tfa9xxx_dsp_init(struct tfa9xxx *tfa9xxx)
{
	...
	if (ret < 0) {
		dev_err(&tfa9xxx->i2c->dev, "%s: Failed starting device\n",
			__func__);
						...
						struct snd_soc_dai *dai)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		if (0 == strncmp(basename, tfa_cnt_profile_name(tfa9xxx, prof), len)) {
			...
			if (!sr)
				dev_info(codec->dev, "%s: Unable to identify supported sample rate\n",
				         __func__);
...
static int tfa9xxx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	dev_info(&tfa9xxx->i2c->dev, "%s: state: %d\n", __func__, mute);
#ifdef Audio2Haptic_refGain
	 if (mute) {
		I2S_run_flag = 0;
	}
		...
		I2S_run_flag = 1;
	dev_dbg(&tfa9xxx->i2c->dev, "%s: I2S_run_flag: %d\n", __func__, I2S_run_flag);
#endif
	...
	if (mute) {
		dev_dbg(&tfa9xxx->i2c->dev, "stream %d tfa9xxx->pstream %d,  tfa9xxx->cstream %d, tfa9xxx->dsp_fw_state %d \n", stream, tfa9xxx->pstream, tfa9xxx->cstream, tfa9xxx->dsp_fw_state);
		/* re-enable haptic f0 tracking when stopping audio playback */
	...
	//dev_info(&tfa9xxx->i2c->dev, "%s: end Chip_run_flag: %d\n", __func__, Chip_run_flag);
	return 0;
}
			       ...
			       struct snd_soc_dai *dai)
{
	...
	dev_info(&tfa9xxx->i2c->dev, "%s: cmd = %d stream = %d\n", __func__,
...
#ifdef NUBIA_HAPTIC_TFA9XXX
static int tfa9xxx_pinctrl_init(struct device *dev,
		struct tfa9xxx *tfa9xxx)
{
	tfa9xxx->pinctrl_info.pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(tfa9xxx->pinctrl_info.pinctrl)) {
		pr_err("%s:tfa9xxx get pinctrl info error.\n", __func__);
		return -EINVAL;
	}

	tfa9xxx->pinctrl_info.pin_active = pinctrl_lookup_state(
					tfa9xxx->pinctrl_info.pinctrl,
					TFA9XXX_PINCTRL_STATE_DEFAULT);
	if (IS_ERR_OR_NULL(tfa9xxx->pinctrl_info.pin_active)) {
		pr_err("%s : tfa9xxx get pin_active info error.\n",__func__);
		return -EINVAL;
	}

	tfa9xxx->pinctrl_info.pin_suspend = pinctrl_lookup_state(
					tfa9xxx->pinctrl_info.pinctrl,
					TFA9XXX_PINCTRL_STATE_DEFAULT);
	if (IS_ERR_OR_NULL(tfa9xxx->pinctrl_info.pin_suspend)) {
		pr_err("%s : tfa9xxx get pin_suspend info error.\n",__func__);
		return -EINVAL;
	}
...
static int tfa9xxx_pinctrl_set_state(
		struct tfa9xxx *tfa9xxx,
		bool active)
{
	int ret = -1;

	if (!tfa9xxx->pinctrl_info.pinctrl ||
			!tfa9xxx->pinctrl_info.pin_active ||
			!tfa9xxx->pinctrl_info.pin_suspend) {
		pr_err("%s : pinctrl is invalid, skip.\n",__func__);
		return ret;
	}
	if (active) {
		ret = pinctrl_select_state(tfa9xxx->pinctrl_info.pinctrl,
				tfa9xxx->pinctrl_info.pin_active);
	} else {
		ret = pinctrl_select_state(tfa9xxx->pinctrl_info.pinctrl,
				tfa9xxx->pinctrl_info.pin_suspend);
			     ...
			     const struct i2c_device_id *id)
{
	...
	mutex_init(&tfa9xxx->power_mutex);
...
#ifdef NUBIA_HAPTIC_TFA9XXX
	ret = tfa9xxx_pinctrl_init(tfa9xxx->dev, tfa9xxx);
	if (ret < 0) {
		pr_err("%s: tfa9xxx pinctrl init failed.\n", __func__);
		//goto pinctrl_init_failed;
	}
	ret = tfa9xxx_pinctrl_set_state(tfa9xxx, true);
	if (ret) {
		pr_err("%s: tfa9xxx pinctrl set state failed.\n", __func__);
		//goto pinctrl_set_failed;
	}
	pr_debug("%s: tfa9xxx pinctrl inited, set state to active successfully.\n", __func__);
#endif
	...
	if (tfa9xxx->mclk) {
		clk_set_rate(tfa9xxx->mclk, 19200000);
		clk_prepare_enable(tfa9xxx->mclk);
		pr_info("tfa set 19.2M clk\n");
	}
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		switch (reg & 0xff) {
#if 0 // TODO: other TFA2 mono devices
		case 0x72: /* tfa9872 */
