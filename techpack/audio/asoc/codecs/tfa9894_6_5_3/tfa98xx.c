...
static void tfa98xxCalibration(struct tfa_device *dev, int mode);

struct tfa_device *dev1 = NULL;
struct tfa_device *dev2 = NULL;
		...
		size_t count, loff_t *ppos)
{
	...
	tfa98xxCalibration(dev1, 1);
	tfa98xxCalibration(dev2, 2);
	return count;
}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_err("%s  selected container profile [%d]\n", __func__, prof_idx);
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	if (tfa98xx->tfa->tfa_family == 2) {
		...
		ret = tfa_dev_stop(tfa98xx->tfa);
		tfa98xx->dsp_init = TFA98XX_DSP_INIT_STOPPED;
		pr_err("[0x%x] tfa_dev_stop return %d in tfa98xx_container_loaded\n",  tfa98xx->i2c->addr, ret);
		mutex_unlock(&tfa98xx->dsp_lock);
	}
#endif
						...
						struct snd_soc_dai *dai)
{
	...
	pr_info("tfa98xx: tfa98xx_startup\n");
	/*
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (no_start) {
		pr_err("no_start parameter set no tfa_dev_start or tfa_dev_stop, returning\n");
		return 0;
	}
...
static int write_impedance(u32 re25, int mode)
{
	struct file *fp;
	mm_segment_t fs;
	loff_t pos;
	...
	if (mode == 1) {
		fp = filp_open("/data/impedance.txt", O_RDWR | O_CREAT | O_TRUNC, 0666);
	} else {
		fp = filp_open("/data/impedance.txt", O_RDWR | O_CREAT | O_APPEND, 0666);
	}
	...
	vfs_write(fp, buf, bytes_write, &pos);
...
static void tfa98xxCalibration(struct tfa_device *dev, int mode) {
	...
	tfa98xx_set_mtp(dev, 1 << TFA_MTPOTC_POS, 1 << TFA_MTPOTC_POS);

	err = tfa_supported_speakers(dev, &spkr_count);
	if (err) {
		pr_err("Getting the number of supported speakers failed : %d", err);
	}
	/* Configure devide into cold boot state for re-calibration */
	err = tfaRunColdStartup(dev, profile);
	if (err) {
	...
	if (!tfa_cf_enabled(dev)) {
		pr_err("It is not possible to calibrate with CF in bypass!");
	}

	mtpex_state = tfa_dev_mtp_get(dev, TFA_MTP_EX);
	tfa_dev_set_state(dev, TFA_STATE_OPERATING);
	if (tfa_dev_mtp_get(dev, TFA_MTP_EX) ) {
		if (mtpex_state == 1) {
			//pr_info("DSP already calibrated.\n Calibration skipped, previous results loaded.");
	...
	else
	{
		...
		/* calibrate */
		err = tfaRunSpeakerCalibration(dev);
		if (err) {
			pr_err("tfa run speaker calibration failed : %d", err);
		}
	}

	if (dev->tfa_family == 2 && !dev->is_probus_device) {
		scaling_factor = 1024;
	}

	imp[0] = dev->mohm[0] *100 / scaling_factor;

	if (spkr_count == 1) {
		pr_err("Calibration value is: %u ohm\n", imp[0]);
	} else {
		imp[1] = dev->mohm[1] * 100 / scaling_factor;
		pr_err("Calibration value is: %u %u ohm\n", imp[0], imp[1]);
	}
	...
	tfa98xx_set_mute(dev, Tfa98xx_Mute_Off);
	...
	if (cal_profile >= 0) {
		...
		err = tfaContWriteProfile(dev, profile, 0);
	}

	/* Always search and apply filters after a startup */
	err = tfa_set_filters(dev, profile);

	/* Save the current profile */
	tfa_dev_set_swprof(dev, (unsigned short)profile);
}
